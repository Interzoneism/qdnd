...
- Full-fidelity + WSL = llvmpipe danger: Under WSL2 without a GPU, Godot uses **llvmpipe** (software Vulkan) which burns ~50% CPU per core and can freeze the OS. Always use `run_autobattle.sh` (which wraps Godot in `nice -n 19` + `timeout`) and never launch full-fidelity Godot directly. Default timeout is 60s. See [AGENTS-FULL-FIDELITY-TESTING.md § WSL / Software Rendering Safety](AGENTS-FULL-FIDELITY-TESTING.md).
