...
namespace QDND.Tools
{
    ...
    public partial class CLIEntryPoint : Node
    {
        ...
        {
            GD.Print("=== AUTO-BATTLE MODE ===");
            GD.Print($"Timestamp: {DateTime.UtcNow:O}");
            GD.Print("Loading CombatArena.tscn so startup matches Play-In-Godot.");
            GetTree().CreateTimer(0.01).Timeout += LoadAutoBattleSceneDeferred;
        }

        private void LoadAutoBattleSceneDeferred()
        {
            var err = GetTree().ChangeSceneToFile("res://Combat/Arena/CombatArena.tscn");
            if (err != Error.Ok)
            {
                GD.PrintErr($"[CLIEntryPoint] Failed to load CombatArena.tscn: {err}");
                ExitWithCode(2);
            }
        }
