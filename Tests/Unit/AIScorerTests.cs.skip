...
namespace QDND.Tests.Unit
{
    ...
    /// Uses mock context to avoid Godot dependencies.
    /// </summary>
    public class AIScorerTests
    {
        ...
        private AIScorer CreateScorer(MockCombatContext mockContext = null, LOSService los = null, HeightService height = null)
        {
            // Create a real context from mock if provided
            if (mockContext != null)
            {
                // We'll pass null for context since we can't instantiate CombatContext in tests
                // The scorer will handle null context gracefully
        ...
        public void ScoreAttack_WithCover_AppliesPenalty()
        {
            ...
            var scorer = CreateScorer(null, los);
            var profile = new AIProfile();
            var action = new AIAction
            {
                ActionType = AIActionType.Attack,
        ...
        public void ScoreAttack_WithHeightAdvantage_AddsBonus()
        {
            ...
            var scorer = CreateScorer(null, null, height);
            var profile = new AIProfile();
            var action = new AIAction
            {
                ActionType = AIActionType.Attack,
